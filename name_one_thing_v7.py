...
=====================================================
Runs NUM_INSTANCES independent Claude instances through a 3-prompt chain
and saves raw results to JSON.

This is the PRIMED-PROMPT version (original v1.3 prompt resurrected at
n=100). Sequential execution (semaphore=1) to avoid rate limits and to
match the execution pattern of the stripped-prompt runs.

Usage:
    export ANTHROPIC_API_KEY="sk-ant-..."
    pip install anthropic
    python name_one_thing_v6.py
"""
...
MAX_TOKENS = 300
CONCURRENCY = 1  # sequential — avoids 429s, matches clean-prompt runs

# ── Prompts (original v1.3 primed wording) ───────────────────────────────
...
async def main():
    ...
    print(f"{'=' * 70}")
    print(f"  Latent Space Convergence Experiment — Data Collection (PRIMED)")
    ...
    filename = f"experiment_results_primed_{timestamp}.json"
